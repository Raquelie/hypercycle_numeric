...
def plot_multiple(n_molecules, sol_all_data, x, inc, num_steps):
    ...
    for i in range(n_molecules):
        ...
        surf = ax.plot_surface(
            T, X, sol_all_data[i, :, :], cmap="viridis", antialiased=True
        )
    ...
    for n in range(1, num_steps):
        ...
        if n % 1000 == 0:
            for i in range(n_molecules):
                ...
                v_min = np.min(sol[i])
                print(
                    f"Step {n}, Molecule {i+1}: max={v_max:.6f}, min={v_min:.6f}, f2={f2:.6f}"
                )
...
def run_model():
    ...
    # 3D Plotting
    fig = plot_multiple(
        cfg["equation_params"]["number_of_molecules"], sol, x, inc, num_steps
    )
